...
import AddAges from './AddAges/AddAges';
import AddAgesAlert from './AddAges/AddAgesAlert';
import AddAgesMessage from './AddAges/AddAgesMessage';
import UpdateAges from './UpdateAges/UpdateAges';
import UpdateAgesAlert from './UpdateAges/UpdateAgesAlert';
import UpdateAgesMessage from './UpdateAges/UpdateAgesMessage';
import DeleteAges from './DeleteAges/DeleteAges';
import DeleteAgesMessage from './DeleteAges/DeleteAgesMessage';
...
class Ages extends React.Component {
    ...
    if (this.state.ages !== this.props.ages) {
      ...
      this.props.ages.map(age => (
        this.setState(prevState => ({
          data: [
            ...prevState.data,
            createData(age.id, age.name, age.description)
          ]
        }))
      ));
    }
  ...
  openUpdateHandler = () => {
    const input = this.state.ages.filter(age => (
...
const mapStateToProps = state => {
  return {
    ...
    add: state.agesReducer.add,
    update: state.agesReducer.update,
    delete: state.agesReducer.delete,
...
const mapDispatchToProps = dispatch => {
  return {
    onFetch: token => dispatch(actions.fetchAges(token)),
    onPost: (age, token) => dispatch(actions.postAges(age, token)),
    onDelete: (id, token) => dispatch(actions.deleteAges(id, token)),
    onUpdate: (id, token, age) => dispatch(actions.updateAges(id, token, age))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(Ages));
