...
import AddRacks from './AddRacks/AddRacks';
import AddRacksAlert from './AddRacks/AddRacksAlert';
import AddRacksMessage from './AddRacks/AddRacksMessage';
import UpdateRacks from './UpdateRacks/UpdateRacks';
import UpdateRacksAlert from './UpdateRacks/UpdateRacksAlert';
import UpdateRacksMessage from './UpdateRacks/UpdateRacksMessage';
import DeleteRacks from './DeleteRacks/DeleteRacks';
import DeleteRacksMessage from './DeleteRacks/DeleteRacksMessage';
...
class Racks extends React.Component {
  ...
  componentDidUpdate() {
    if (this.state.racks !== this.props.racks) {
      ...
      this.props.racks.map(rack => (
        this.setState(prevState => ({
          data: [
            ...prevState.data,
            createData(rack.id, rack.name, rack.description)
          ]
        }))
      ));
    }
  ...
  openUpdateHandler = () => {
    const input = this.state.racks.filter(rack => (
      rack.id === this.state.selected[0]
    ));
...
const mapStateToProps = state => {
  return {
    ...
    add: state.racksReducer.add,
    update: state.racksReducer.update,
    delete: state.racksReducer.delete,
...
const mapDispatchToProps = dispatch => {
  return {
    onFetch: token => dispatch(actions.fetchRacks(token)),
    onPost: (rack, token) => dispatch(actions.postRacks(rack, token)),
    onDelete: (id, token) => dispatch(actions.deleteRacks(id, token)),
    onUpdate: (id, token, rack) => dispatch(actions.updateRacks(id, token, rack))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(Racks));
