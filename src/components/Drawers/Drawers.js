...
import AddDrawers from './AddDrawers/AddDrawers';
import AddDrawersAlert from './AddDrawers/AddDrawersAlert';
import AddDrawersMessage from './AddDrawers/AddDrawersMessage';
import UpdateDrawers from './UpdateDrawers/UpdateDrawers';
import UpdateDrawersAlert from './UpdateDrawers/UpdateDrawersAlert';
import UpdateDrawersMessage from './UpdateDrawers/UpdateDrawersMessage';
import DeleteDrawers from './DeleteDrawers/DeleteDrawers';
import DeleteDrawersMessage from './DeleteDrawers/DeleteDrawersMessage';
    ...
    if (this.state.drawers !== this.props.drawers) {
      ...
      this.props.drawers.map(drawer => (
        this.setState(prevState => ({
          data: [
            ...prevState.data,
            createData(drawer.id, drawer.name, drawer.rack_id, drawer.description)
          ]
        }))
      ));
    }

    if (this.state.racks !== this.props.racks) {
      this.setState({
...
const mapStateToProps = state => {
  return {
    ...
    racks: state.racksReducer.racks,
    add: state.drawersReducer.add,
    update: state.drawersReducer.update,
    delete: state.drawersReducer.delete,
...
const mapDispatchToProps = dispatch => {
  return {
    onFetch: token => dispatch(actions.fetchDrawers(token)),
    onFetchRacks: token => dispatch(actions.fetchRacks(token)),
    onPost: (drawer, token) => dispatch(actions.postDrawers(drawer, token)),
    onDelete: (id, token) => dispatch(actions.deleteDrawers(id, token)),
    onUpdate: (id, token, drawer) => dispatch(actions.updateDrawers(id, token, drawer))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(Drawers));
