...
import AddAcquisitions from './AddAcquisitions/AddAcquisitions';
import AddAcquisitionsAlert from './AddAcquisitions/AddAcquisitionsAlert';
import AddAcquisitionsMessage from './AddAcquisitions/AddAcquisitionsMessage';
import UpdateAcquisitions from './UpdateAcquisitions/UpdateAcquisitions';
import UpdateAcquisitionsAlert from './UpdateAcquisitions/UpdateAcquisitionsAlert';
import UpdateAcquisitionsMessage from './UpdateAcquisitions/UpdateAcquisitionsMessage';
import DeleteAcquisitions from './DeleteAcquisitions/DeleteAcquisitions';
import DeleteAcquisitionsMessage from './DeleteAcquisitions/DeleteAcquisitionsMessage';
...
class Acquisitions extends React.Component {
  state = {
    order: 'asc',
    orderBy: 'id',
    selected: [],
    ...
    if (this.state.acquisitions !== this.props.acquisitions) {
      ...
      this.props.acquisitions.map(acquisition => (
        this.setState(prevState => ({
          data: [
            ...prevState.data,
            createData(acquisition.id, acquisition.name, acquisition.description)
          ]
        }))
      ));
    }
  ...
  openUpdateHandler = () => {
    const input = this.state.acquisitions.filter(acquisition => (
      acquisition.id === this.state.selected[0]
    ));
...
const mapStateToProps = state => {
  return {
    ...
    add: state.acquisitionsReducer.add,
    update: state.acquisitionsReducer.update,
    delete: state.acquisitionsReducer.delete,
    loading: state.acquisitionsReducer.loading
  };
};

const mapDispatchToProps = dispatch => {
  return {
    onFetch: token => dispatch(actions.fetchAcquisitions(token)),
    onPost: (acquisition, token) => dispatch(actions.postAcquisitions(acquisition, token)),
    onDelete: (id, token) => dispatch(actions.deleteAcquisitions(id, token)),
    onUpdate: (id, token, acquisition) => dispatch(actions.updateAcquisitions(id, token, acquisition))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(Acquisitions));
