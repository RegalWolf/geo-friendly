...
import AddScales from './AddScales/AddScales';
import AddScalesAlert from './AddScales/AddScalesAlert';
import AddScalesMessage from './AddScales/AddScalesMessage';
import UpdateScales from './UpdateScales/UpdateScales';
import UpdateScalesAlert from './UpdateScales/UpdateScalesAlert';
import UpdateScalesMessage from './UpdateScales/UpdateScalesMessage';
import DeleteScales from './DeleteScales/DeleteScales';
import DeleteScalesMessage from './DeleteScales/DeleteScalesMessage';
import Spinner from './../Spinner/Spinner';

function createData(id, name, amount, description) {
  return { id, name, amount, description };
}
...
class Scales extends React.Component {
    ...
    if (this.state.scales !== this.props.scales) {
      ...
      this.props.scales.map(scale => (
        this.setState(prevState => ({
          data: [
            ...prevState.data,
            createData(scale.id, scale.name, scale.amount, scale.description)
          ]
        }))
      ));
    }
  ...
  addValidationHandler = () => {
    ...
    if (!this.state.add.input.amount) {
      amount = 'amount is required';
    } else if (this.state.add.input.amount !== '10') {
      amount = 'The amount must be 10';
    }
  ...
  toggleAddAlertHandler = async () => {
    ...
    if ((!this.state.add.inputMessage.name) 
      ...
      && (!this.state.add.inputMessage.amount)) {
      this.setState(prevState => ({
        add: {
          ...prevState.add,
  ...
  openUpdateHandler = () => {
    const input = this.state.scales.filter(scale => (
      scale.id === this.state.selected[0]
    ));
  ...
  updateValidationHandler = () => {
    ...
    if (!this.state.update.input.amount) {
      amount = 'amount is required';
    } else if (this.state.update.input.amount.length < 4) {
      amount = 'The amount must be at least 4 characters';
    }
  ...
  toggleUpdateAlertHandler = async () => {
    ...
    if ((!this.state.update.inputMessage.name) 
      && (!this.state.update.inputMessage.amount)
...
const mapStateToProps = state => {
  return {
    ...
    add: state.scalesReducer.add,
    update: state.scalesReducer.update,
    delete: state.scalesReducer.delete,
    loading: state.scalesReducer.loading
  };
};

const mapDispatchToProps = dispatch => {
  return {
    onFetch: token => dispatch(actions.fetchScales(token)),
    onPost: (scale, token) => dispatch(actions.postScales(scale, token)),
    onDelete: (id, token) => dispatch(actions.deleteScales(id, token)),
    onUpdate: (id, token, scale) => dispatch(actions.updateScales(id, token, scale))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(Scales));
