...
import AddIslands from './AddIslands/AddIslands';
import AddIslandsAlert from './AddIslands/AddIslandsAlert';
import AddIslandsMessage from './AddIslands/AddIslandsMessage';
import UpdateIslands from './UpdateIslands/UpdateIslands';
import UpdateIslandsAlert from './UpdateIslands/UpdateIslandsAlert';
import UpdateIslandsMessage from './UpdateIslands/UpdateIslandsMessage';
import DeleteIslands from './DeleteIslands/DeleteIslands';
import DeleteIslandsMessage from './DeleteIslands/DeleteIslandsMessage';
...
class Islands extends React.Component {
    ...
    if (this.state.islands !== this.props.islands) {
      ...
      this.props.islands.map(island => (
        this.setState(prevState => ({
          data: [
            ...prevState.data,
            createData(island.id, island.name, island.description)
          ]
        }))
      ));
    }
  ...
  openUpdateHandler = () => {
    const input = this.state.islands.filter(island => (
      island.id === this.state.selected[0]
    ));
...
const mapStateToProps = state => {
  return {
    ...
    add: state.islandsReducer.add,
    update: state.islandsReducer.update,
    delete: state.islandsReducer.delete,
...
const mapDispatchToProps = dispatch => {
  return {
    onFetch: token => dispatch(actions.fetchIslands(token)),
    onPost: (island, token) => dispatch(actions.postIslands(island, token)),
    onDelete: (id, token) => dispatch(actions.deleteIslands(id, token)),
    onUpdate: (id, token, island) => dispatch(actions.updateIslands(id, token, island))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(Islands));
